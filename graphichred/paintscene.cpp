...
void PaintScene::setPenOpacity(qreal opacity)
{
    penOpacity = opacity;
}

void PaintScene::setEraser(bool enabled)
{
    isEraser = enabled;
    if (enabled) {
...
{
    if (m_typeFigure == LineType)
    {
        QColor color = isEraser ? Qt::white : currentColor;
        color.setAlphaF(penOpacity);
        addEllipse(event->scenePos().x() - penSize/2,
                   event->scenePos().y() - penSize/2,
                   penSize,
                   penSize,
                   QPen(Qt::NoPen),
                   QBrush(color));
...
void PaintScene::mouseMoveEvent(QGraphicsSceneMouseEvent *event)
{
    if (m_typeFigure == LineType)
    {
        ...
        addLine(previousPoint.x(),
                ...
                event->scenePos().x(),
                event->scenePos().y(),
                QPen(color, penSize, Qt::SolidLine, Qt::RoundCap));
        previousPoint = event->scenePos();
    } else if (tempFigure) {
